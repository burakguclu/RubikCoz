...
export default function SolutionDisplay({ moves, currentStep, onStepClick, isPlaying, onPlay, onPause, onReset, onPrev, onNext }) {
  ...
  return (
    <div className="glass-card p-5 animate-fade-in">
      ...
      <div className="flex gap-2 justify-center mb-4">
        ...
        <button className="btn-secondary text-sm px-3 py-2" onClick={onPrev} title="Önceki">
          ◀
        </button>
        {isPlaying ? (
          <button className="btn-danger text-sm px-4 py-2" onClick={onPause} title="Duraklat">
            ⏸ Duraklat
          </button>
        ) : (
          <button className="btn-success text-sm px-4 py-2" onClick={onPlay} title="Oynat">
            ▶ Oynat
          </button>
        )}
        <button className="btn-secondary text-sm px-3 py-2" onClick={onNext} title="Sonraki">
          ▶
        </button>
      ...
      {currentStep > 0 && currentStep <= moves.length && (
        <div className="text-center mb-3 text-sm text-white/70">
          <span className="font-semibold text-white">{moves[currentStep - 1]}</span>
          {' — '}
